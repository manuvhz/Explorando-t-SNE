...
const InfoCard: React.FC<{ title: string; subtitle: string; children: React.ReactNode; delay: number }> = ({ title, subtitle, children, delay }) => (
    <div 
        className="group relative p-8 bg-white dark:bg-light-navy rounded-lg shadow-lg hover:shadow-2xl dark:hover:shadow-neon-turquoise/20 transition-all duration-300 transform hover:-translate-y-2 animate-fade-in-stagger"
    ...
    >
      ...
      <p className="text-slate dark:text-slate">{children}</p>
    </div>
);
