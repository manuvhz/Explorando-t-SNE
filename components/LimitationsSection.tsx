...
const LimitationCard: React.FC<{ item: LimitationData, delay: number }> = ({ item, delay }) => {
    ...
    return (
        <div 
            className="bg-white dark:bg-light-navy p-6 rounded-lg shadow-md hover:shadow-xl dark:hover:shadow-neon-turquoise/10 transition-all duration-300 flex items-start gap-4 animate-fade-in-stagger"
            style={{ animationDelay: `${delay}ms` }}
        >
            <Icon className={`h-8 w-8 shrink-0 mt-1 ${iconColor}`} />
            <div>
                <h3 className={`text-xl font-bold ${titleColor} mb-2`}>{item.title}</h3>
                <p className="text-slate dark:text-slate">{item.description}</p>
            </div>
        </div>
    );
};
