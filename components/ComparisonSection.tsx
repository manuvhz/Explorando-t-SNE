...
const ComparisonCard: React.FC<{ data: ComparisonData, delay: number }> = ({ data, delay }) => (
    <div 
        className="bg-white dark:bg-light-navy rounded-lg shadow-lg p-6 flex flex-col h-full animate-fade-in-stagger"
        style={{ animationDelay: `${delay}ms` }}
    >
        <h3 className="text-xl font-bold text-slate-900 dark:text-light-slate mb-2">{data.title}</h3>
        <p className="text-sm text-slate dark:text-slate mb-4 flex-grow">{data.description}</p>
        
        <div className="bg-slate-100 dark:bg-deep-blue/50 rounded-md p-2 mb-4">
        ...
        <div>
            ...
            <ul className="space-y-1 mb-4">
                {data.pros.map(pro => (
                    <li key={pro} className="flex items-start text-sm text-slate dark:text-slate"><CheckCircleIcon /> {pro}</li>
                ))}
            </ul>
            <h4 className="font-semibold mb-2 text-slate-900 dark:text-light-slate">Desventajas</h4>
            <ul className="space-y-1">
                {data.cons.map(con => (
                    <li key={con} className="flex items-start text-sm text-slate dark:text-slate"><XCircleIcon /> {con}</li>
                ))}
            </ul>
        </div>
    </div>
