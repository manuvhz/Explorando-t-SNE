...
const ApplicationCard: React.FC<{ card: ApplicationCardData; onOpenModal: () => void, delay: number }> = ({ card, onOpenModal, delay }) => {
    ...
    return (
        <div 
            className="bg-white dark:bg-light-navy p-6 rounded-lg shadow-lg flex flex-col group hover:-translate-y-2 transition-transform duration-300 animate-fade-in-stagger"
        ...
        >
            ...
            <p className="text-slate dark:text-slate flex-grow">{card.description}</p>
        ...
        <AnimatedSection id="applications">
            ...
            {modalData && (
                <Modal onClose={() => setModalData(null)}>
                    ...
                    <p className="text-slate dark:text-slate">{modalData.details}</p>
                </Modal>
            )}
        </AnimatedSection>
    );
