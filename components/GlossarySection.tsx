...
const GlossaryCard: React.FC<{ term: string, definition: string, delay: number }> = ({ term, definition, delay }) => (
    <div 
        className="bg-white dark:bg-light-navy p-6 rounded-lg shadow-md hover:shadow-xl dark:hover:shadow-neon-turquoise/10 transition-all duration-300 transform hover:-translate-y-1 animate-fade-in-stagger"
        style={{ animationDelay: `${delay}ms` }}
    >
      <h3 className="text-xl font-bold text-neon-turquoise mb-2">{term}</h3>
      <p className="text-slate dark:text-slate">{definition}</p>
    </div>
);
